...
class CodegenC(CodegenCBase):
    ...
    def get_conv_info(self,call):
        ...
        while len(op_list) > 0:
            ...
            if hasattr(op, "op") and op.op.name == "nn.conv2d":
                conv2d_info["PAD"] = op.attrs["padding"][0]
                conv2d_info["M"] = int(op.attrs["channels"])
                conv2d_info["R"] = int(op.attrs["kernel_size"][0])
                conv2d_info["S"] = int(op.attrs["kernel_size"][1])
                conv2d_info["U"] = int(op.attrs["strides"][0])
    ...
    def visit_call(self, call):
        ...
        for arg, pname in zip(call.args, tvm_auto_args_NOTES[func_name]):
            if isinstance(arg, Constant):
                out_obj = self.visit_constant(arg)[0]
                out_obj.data = arg.data
                parameters[pname] = (out_obj, True)
            else:
                out_obj = self.visit_expr(arg)[0]
                parameters[pname] = (out_obj, False)
...
class CSourceCodegen(CSourceModuleCodegenBase):
    ...
    def gen_weight(self, const_vars):
        ...
        for i, const_var in enumerate(const_vars):
            ...
            # get weight size
            flattened_size = int(np.prod(data_numpy.shape))
            const_var.size = flattened_size

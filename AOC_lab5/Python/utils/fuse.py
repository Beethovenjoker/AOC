...
def fuse_conv2d_bias_add_relu_max_pool2d():
    ...
    # conv2d -> bias_add -> relu -> quant -> cast
    conv2d_op = dfp.is_op("nn.conv2d")(dequantized_i,dequantized_w)
    bias_add_op = dfp.is_op("nn.bias_add")(conv2d_op, dequantized_b)
    relu_op = dfp.is_op("nn.relu")(bias_add_op)
    quantize_op = dfp.is_op("qnn.quantize")(relu_op,dfp.wildcard(),dfp.wildcard())
    ...
    # dequant -> maxpool -> quant
    dequant_op = dfp.is_op("qnn.dequantize")(cast_op,dfp.wildcard(),dfp.wildcard())
    maxpool_op = dfp.is_op("nn.max_pool2d")(dequant_op)
    quantize_op = dfp.is_op("qnn.quantize")(maxpool_op,dfp.wildcard(),dfp.wildcard())

    # cast
    cast_op2 = dfp.is_op("cast")(quantize_op)
...
def fuse_flatten():
    ...
    # flatten
    flatten = dfp.is_op("nn.batch_flatten")(dequantized_i)

    # quantize
    quantize_op = dfp.is_op("qnn.quantize")(flatten, dfp.wildcard(), dfp.wildcard())
...
def quantize():
    ...
    quantize_op = dfp.is_op("qnn.quantize")(i, dfp.wildcard(), dfp.wildcard())
